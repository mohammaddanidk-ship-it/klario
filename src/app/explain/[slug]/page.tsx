...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const desc = rec.isPhishing ? `Klarium Shield explains how to identify ${rec.docType} fraud. Free AI scam detection. No account required.` : `Klarium explains what a ${rec.docType} means in plain language. Free AI document clarity. No account required.`;
  return {
    title,
    description: desc,
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F5F5F7", fontFamily:"Inter,system-ui,-apple-system,sans-serif", color:"#1D1D1F", WebkitFontSmoothing:"antialiased" }}>
      ...
      <main style={{ maxWidth:760, margin:"0 auto", padding:"40px 20px 80px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:28, flexWrap:"wrap" }}><span style={{ fontSize:12, color:"#6E6E73" }}>Explained by Klarium AI · {new Date(rec.createdAt).toLocaleDateString("en-US", { year:"numeric", month:"long", day:"numeric" })}</span>{rec.count > 1 && <span style={{ fontSize:12, fontWeight:600, color:accent, background:accentBg, border:`1px solid ${accentBr}`, padding:"3px 10px", borderRadius:20 }}>Explained {rec.count} times</span>}{rec.views > 0 && <span style={{ fontSize:12, color:"#9CA3AF" }}>{rec.views.toLocaleString()} views</span>}</div>
        ...
        <div style={{ background:"#fff", border:"1px solid #E5E7EB", borderRadius:14, padding:"22px 24px", marginBottom:20, boxShadow:"0 1px 4px rgba(0,0,0,.05)" }}>
          ...
          <div style={{ marginTop:16, paddingTop:12, borderTop:"1px solid #F3F4F6", display:"flex", alignItems:"flex-start", gap:6 }}><span style={{ fontSize:11, flexShrink:0 }}>🔒</span><p style={{ fontSize:11, color:"#9CA3AF", lineHeight:1.5 }}>{rec.isPhishing ? "For suspected fraud, report to your bank and local cybercrime authorities immediately." : "For legal, medical, or financial decisions, always consult a qualified professional."}</p></div>
        </div>

        {snippets.length > 0 && <div style={{ background:"#fff", border:"1px solid #E5E7EB", borderRadius:14, padding:"20px 24px", marginBottom:20 }}><h2 style={{ fontSize:14, fontWeight:700, color:"#1D1D1F", marginBottom:14 }}>Real examples people have submitted</h2><div style={{ display:"flex", flexDirection:"column", gap:10 }}>{snippets.map((snippet, i) => <div key={i} style={{ padding:"12px 14px", background:"#F9FAFB", borderRadius:8, border:"1px solid #E5E7EB" }}><p style={{ fontSize:12, color:"#6B7280", fontStyle:"italic", lineHeight:1.6 }}>&ldquo;{snippet}{snippet.length >= 200 ? "…" : ""}&rdquo;</p></div>)}</div><p style={{ fontSize:11, color:"#9CA3AF", marginTop:12 }}>These examples were submitted by real users and explained by Klarium AI.</p></div>}

        <div style={{ background:"#fff", border:"1px solid #E5E7EB", borderRadius:14, padding:"20px 24px", marginBottom:20 }}><h2 style={{ fontSize:14, fontWeight:700, color:"#1D1D1F", marginBottom:14 }}>Common questions about {rec.docType}s</h2>{(rec.isPhishing ? [{ q:`How do I know if a ${rec.docType} is a scam?`, a:`Look for urgent language, suspicious links, requests for personal information, and sender addresses that don't match the official organisation. Klarium Shield can analyse any message instantly.` },{ q:`What should I do if I received a suspicious ${rec.docType}?`, a:`Do not click any links or provide any information. Contact the organisation directly using a phone number from their official website. Report the message to your bank's fraud team.` }] : [{ q:`What is a ${rec.docType}?`, a:`A ${rec.docType} is a formal document that contains legal or official information. Klarium explains what it means in plain language so you can understand your rights and obligations.` },{ q:`Do I need a lawyer to understand a ${rec.docType}?`, a:`For major decisions, yes. But Klarium helps you understand the basics first — what the document says, what to watch out for, and what questions to ask your lawyer.` },{ q:`Is it safe to upload a ${rec.docType} to Klarium?`, a:`Yes. Klarium processes your document privately and never stores it on our servers. Your document is analysed in real-time and immediately discarded.` }]).map(({ q, a }, i) => <div key={i} style={{ marginBottom:i < 1 ? 14 : 0, paddingBottom:i < 1 ? 14 : 0, borderBottom:i < 1 ? "1px solid #F3F4F6" : "none" }}><p style={{ fontSize:14, fontWeight:600, color:"#1D1D1F", marginBottom:5 }}>{q}</p><p style={{ fontSize:13, color:"#6E6E73", lineHeight:1.6 }}>{a}</p></div>)}</div>

        <div style={{ padding:"28px 24px", borderRadius:14, background:"#0A1628", textAlign:"center", color:"#fff" }}><p style={{ fontWeight:800, fontSize:22, marginBottom:8, letterSpacing:"-0.03em" }}>Have a confusing document?</p><p style={{ fontSize:15, color:"rgba(255,255,255,.55)", marginBottom:22, lineHeight:1.6 }}>Klarium explains any document in plain language. Free, private, instant.</p><Link href="/#upload" style={{ display:"inline-block", background:accent, color:"#fff", padding:"13px 30px", borderRadius:10, fontWeight:700, fontSize:15 }}>Explain My Document →</Link><p style={{ fontSize:11, color:"rgba(255,255,255,.3)", marginTop:14 }}>No account required · Documents never stored · Free forever</p></div>
      </main>
